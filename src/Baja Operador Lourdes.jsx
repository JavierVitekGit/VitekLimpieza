        ...
        get(child(dbRef,'Operador')).then((snapshot) => {
          if(snapshot.exists()){
            ...
            arrayClientCl.forEach((other => {
              datos.forEach(iter => {
                if (iter.cl == other){
                    datosO.push(iter.name)

                    console.log("Lourdes:",datosO)
                    datosO.sort()
                }
            })
          }))
...
{datosO.map((item) => <option value={item}>{item}</option> )}
</select> 
