...
const ReasignacionL = (reasignacion) => {
    ...
    const [arrayCl,setArrayCl] = useState([]);
...
<div className="SideOlgaBb">
      ...
      <div className="roH">
        ...
        <i id="ri" class="bi bi-arrow-down-up"></i>
          Reasignación del Operador Lourdes
          </h1>
    ...
    <div className="container">
        ...
        {arrayRfc.map((item)=> <option value={item}>{item}</option>)}    
        </select>
          <br/>
        ...
        <select onClick={forceUpdate} value={cliente} onChange={v=>{setCliente(v.target.value)}}>
            ...
            {arrayCl.map((item,i)=> <option>{item}</option>)}
