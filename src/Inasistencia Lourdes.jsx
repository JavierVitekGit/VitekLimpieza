...
const InasistenciaL = (inasistencia) => {
...
<div className="SideOlgaB">
  ...
  <h1 className="inasistenciaTitle">Lista de Inasistencia Lourdes</h1>
  <h1 className="dateI">{dia + "-" + mes + "-" + año}</h1>
</div>

<input className="filter" type="text" placeholder="..." value={search} onChange={handleSearchChange} ></input>
...
{/* {filtered.map((person) => {
    return (
        <p className="listTest" key={person.id}>
            {person.clienteC} - {person.name}
        </p>
    );
})} */}
